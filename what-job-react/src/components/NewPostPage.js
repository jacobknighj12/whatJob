...
export function NewPostPage() {
  const [intro, setIntro] = useState(
    "Short introduction for yourself, how long you have worked in this position and an example of a great day and a bad day at your job."
  );
  const [day_to_day, setDay_to_day] = useState(
    "Go over the boring/exciting stuff that you often overlook but most of your days consist of"
  );
  const [career_path, setCareer_path] = useState(
    "How did you get to this position, study, luck, hardship, Nepotism. As well as ways that you know that enable a person to enter this field today."
  );
  const [industry_growth, setIndustry_growth] = useState(
    "How fast is this industry growing, or is it shrinking? do they have good or bad entry into the industry due to new things happening?."
  );
  const [work_life_balance, setWork_life_balance] = useState(
    "how much time do you have to yourself after work? do you do extra hours constantly? do you have lots of overtime? can you get time off easily? how many hours from getting up till you get home is your average day? including travel."
  );
  ...
  function handleSubmit(event) {
    ...
    data.append(
      "expected_salary_range_from",
      `${event.target.expected_salary_range_from.value}`
    );
    data.append(
      "expected_salary_range_to",
      `${event.target.expected_salary_range_to.value}`
    );
